...
class Form {
    ...
    public function write_to_db(){
        ...
        $sql = static::get_pdo()->prepare('INSERT INTO `' . $this->table . '` (`name`, `lastname`, `email`, `phone`, `subject_id`, `payment_id`, `mailing`, `created_at`, 
            `updated_at`, `deleted_at`) VALUES (?,?,?,?,?,?,?,?,?,?);');
        $sql->execute(array($this->name, $this->lastname, $this->email, $this->phone, $t, $p, $this->check, $this->dateCreate, $this->dateUpdate, $this->dateDelete));
    ...
    public function form_fill() {
        ...
        $message = new Flash;
        $message->set('<h2>Ваша заявка принята!</h2><p><a href="index.php">Заполнить заново</a></p>');
    ...
    public function read_from_db(){
        $sql = static::get_pdo()->prepare('SELECT t1.id, t1.name, lastname, email, phone, t2.sub_name, t3.pay_name, mailing, created_at, updated_at, deleted_at FROM `' . $this->table . '` t1,`' . $this->table2 . '` t2,`' . $this->table3 . '` t3 WHERE t1.subject_id = t2.id AND t1.payment_id = t3.id AND `deleted_at` is NULL;');
        $sql->execute();

        $objects = [];

        while ($object = $sql->fetchObject(static::class)) {
            $str = $object->id . " | " . $object->name . " | " . $object->lastname . " | " . $object->email . " | " . $object->phone . " | " . $object->sub_name . " | " . $object->pay_name . " | "  . $object->check.  " | " . $object->created_at . " | " . $object->updated_at;
